...
def validate_excel(excel, strain, excelDict):
    ...
    for sheet in SHEETS:
        if sheet["name"] in sheets:
            ...
            if sheet["name"] not in [
                ...
                errors.append(
                    Error(
                        f"The '{sheet['name']}' sheet is missing. Please check the provided excel template",
                        sheet["name"],
                    )
                )
    # if len(errors) == 0:
    ...
    error_log.write(
        os.path.abspath(os.path.join(os.path.dirname(mirri.__file__), "..", "logs"))
    )
    return error_log
...
def validate_sheet(sheetEx, sheet):
    ...
    for col in sheet["columns"]:
        if col[0] not in headers and col[1]:
            errors.append(
                Error(
                    f"The '{col[0]}' is a mandatory field. The column can not be empty.",
                    col[0],
                )
            )
    return errors
    ...
    for field in MIRRI_FIELDS:
        if field["label"] in types2:
            if types2[field["label"]] != field["type"]:
                error.append(
                    Error(f'The "{field["label"]}" column has an invalide data type.')
                )
